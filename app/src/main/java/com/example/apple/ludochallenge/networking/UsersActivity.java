...
//        mUsersDatabase = FirebaseDatabase.getInstance().getReference().child("Users").child(user_id);
                    ...
                    holder.mView.findViewById(R.id.challenge_btn).setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(final View view) {
                            ...
                            if (view.getTag().equals("challenged")) {
                                ...
                                mChallenge_database.child(mCurrent_user.getUid()).child(user_id).removeValue();
                                gameStarted.push().child("started_games").push().child(mCurrent_user.getUid()).push().child(user_id).push().child("players").setValue("2").addOnCompleteListener(new OnCompleteListener<Void>() {
                                    @Override
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            ...
                                            mChallenge_database.child(mCurrent_user.getUid()).child(user_id).removeValue().addOnCompleteListener(
                                                    new OnCompleteListener<Void>() {
                                                        @Override
                                                        public void onComplete(@NonNull Task<Void> task) {
                                                            gameStarted.child(mCurrent_user.getUid()).child(user_id).child("turn").setValue(user_id);
                                                            gameStarted.child(mCurrent_user.getUid()).child(user_id).child("dice_value").setValue(4);
                                                            gameStarted.child(mCurrent_user.getUid()).child(user_id).child("piece").setValue(0);
                                                            gameStarted.child(mCurrent_user.getUid()).child(user_id).child("firstUID").setValue(user_id);
                                                            gameStarted.child(mCurrent_user.getUid()).child(user_id).child("secondUID").setValue(mCurrent_user.getUid()).addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                @Override
                                                                public void onComplete(@NonNull Task<Void> task) {
                                                                    if(task.isSuccessful()) {
                                                                        ...
                                                                        intent.putExtra(LudoActivity.REFERENCE, gameStarted.child(mCurrent_user.getUid()).child(user_id).toString());
    ...
    public void onBackPressed() {
        Intent intent = new Intent(getApplicationContext(), BaseActivity.class);
        intent.putExtra("activity", PlayActivity.class.getName());
